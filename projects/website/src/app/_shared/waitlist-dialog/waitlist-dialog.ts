...
export class WaitlistDialog {
  ...
  protected readonly roles: { key: WaitlistRole; label: string; icon: string }[] = [
    { key: 'instructor', label: $localize`:@@waitlist.role.lead:I lead activities`, icon: 'pi pi-flag' },
    { key: 'user', label: $localize`:@@waitlist.role.join:I join activities`, icon: 'pi pi-heart' },
  ];
      ...
      .subscribe({
        ...
        error: () => {
          ...
          this._messageService.add({
            ...
            detail: $localize`:@@waitlist.toast.error.detail:Could not join the waitlist. Please try again.`,
            life: 5000,
          });
        },
      });
