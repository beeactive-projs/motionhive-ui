...
/** Icon names rendered as static `name="…"` attributes across the feature. */
function iconNamesInTemplates(): Set<string> {
...
describe('workouts config', () => {
  ...
  it('registers every icon its templates render', () => {
    const registered = new Set(Object.keys(WORKOUT_ICONS).map(kebab));
    for (const name of iconNamesInTemplates()) {
